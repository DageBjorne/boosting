...
v1_list = [0.005, 0.01, 0.03, 0.04]
target_tree_size_list = [1,2,3,4]
source_tree_size_list = [1,2,3,4]
decay_factor_list = [0.97, 0.99, 0.995, 0.997]
...
def create_train_test_split(test_size = 0.25, train_size = 0.5, target_col = "Dgv", use_sweden = True, test_seed = 1, train_seed = 1):
    ...
    data_train_sweden, data_test_sweden_1 = train_test_split(data_train_sweden_1, test_size = test_size, random_state = test_seed) #random_state is fixed

    data_train_sweden_1, data_test = train_test_split(data_train_sweden, test_size = 1-train_size, random_state = train_seed) #random state is not fixed, data_test unused
    ...
    if use_sweden == False:
        ...
        data_train_sweden, data_test_sweden_1 = train_test_split(data_latvia, test_size = test_size, random_state = test_seed) #random_state is fixed
...
def boosted_prediction(a_test, a_train, b_train, model_tray_clf, model_tray_clfhat, 
                       leaf_gammas_tray, leaf_gammashats_tray, v1, v2, alpha_tray):
    ...
    for i, clf in enumerate(model_tray_clf):
        ...
        leaves_clf_train_test = np.concatenate((leaves_clf_train, leaves_clf_test))
        leaves_clfhat_train_test = np.concatenate((leaves_clfhat_train, leaves_clfhat_test))

        indexed_leaves_clf_train_test = map_leaves_to_number(leaves_clf_train_test)
        indexed_leaves_clfhat_train_test = map_leaves_to_number(leaves_clfhat_train_test)
        ...
        for index in np.unique(indexed_leaves_clf_test):
            F_pred[indexed_leaves_clf_test == index] += v1*leaf_gamma[index]*(1-alpha)
...
def LSTreeBoost(a, b, ahat, bhat, x_test, y_test,
                v1=0.1, v2=0.1, epochs=100,
                target_tree_size=2, source_tree_size=2,
                alpha_0 = 1.0,
                decay_factor=0.99,
                eval = True):
    ...
    for m in range(epochs):
        ...
        clf = DecisionTreeRegressor(max_depth=target_tree_size, min_samples_leaf=25)
        clf.fit(a, b_residuals)

        clfhat = DecisionTreeRegressor(max_depth=source_tree_size, min_samples_leaf=25)
       ...
       # print(f"Epoch {m}: unique_leaves_clfhat size = {len(unique_leaves_clfhat)}")
        ...
        for index in np.unique(indexed_leaves_clfhat):
            ...
            F_source[indexed_leaves_clfhat == index] += v2*leaf_gammahat[index] #not used now. I add it so we can update residuals for source separately, if ever needed
...
df = pd.DataFrame(columns = ['id', 'train_seed', 'test_seed', 'train_size', 'test_size', 
                             'v', 'target_tree_size', 'source_tree_size',
    ...
    for test_seed in test_seed_list:
        for train_size in train_size_list:
            for test_size in test_size_list:
                ahat_train, bhat_train, a_train, b_train, x_test, y_test = create_train_test_split(test_size = test_size,
                                                                                                train_size = train_size, target_col = "Dgv", 
                                                                                                use_sweden = True, test_seed = test_seed, 
                            ...
                            for epochs in epoch_list:
                                for decay_factor in decay_factor_list:
                                    for alpha_0 in alpha_0_list:
                                        losses_target, losses_source, losses_test, epochs_test, leaf_gammas_tray, leaf_gammashats_tray, model_tray_clf, model_tray_clfhat, alpha_tray = LSTreeBoost(a_train, b_train, ahat_train, bhat_train, x_test, y_test,
                                                                                                                                                                                                    v1=v1, v2=v2, epochs=epochs,    
                                        ...
                                        preds = boosted_prediction(x_test, a_train, b_train, 
                                                                ...
                                                                v1=v1, v2=v2, alpha_tray=alpha_tray)
                                        test_rmse = compute_rmse(preds, y_test) 
                                        df.loc[len(df)] = [int(i), train_seed, test_seed, len(a_train), len(x_test), v1, target_tree_size, 
                                                               source_tree_size, epochs, decay_factor, alpha_0, test_rmse]
                                        ...
                                        plt.savefig(os.path.join('imgs', str(int(i)) + '.jpg'))
                                        plt.close('all')
